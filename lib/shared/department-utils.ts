...
export function getDepartmentConfig(name: string): Department | undefined {
  ...
  for (const dept of DEPARTMENTS) {
    const subspecialty = dept.subspecialties && dept.subspecialties.find((sub: Subspecialty) => sub.name === name);
    if (subspecialty) {
      return subspecialty;
    }
  }
...
export function getSubspecialtyConfig(name: string, departmentName: string): any | undefined {
  ...
  return deptConfig?.subspecialties?.find((sub: Subspecialty) => sub.name === name);
} 