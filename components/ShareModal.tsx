    ...
    <div className={`fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 transition-all duration-700 ease-in-out ${isTransitioning ? 'opacity-0' : 'opacity-100'}`}>
      <div className={`bg-white dark:bg-slate-800 rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl max-h-[90vh] overflow-y-auto transition-all duration-700 ease-in-out ${isTransitioning ? 'scale-90 opacity-0 translate-y-4' : 'scale-100 opacity-100 translate-y-0'}`}>
        ...
        <div className="text-center mb-6">
          <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2 transition-all duration-500">
            {isTransitioning ? 'Returning to Home...' : 'Share Your Achievement'}
          </h3>
          <p className="text-slate-500 dark:text-slate-400 text-sm transition-all duration-500">
        ...
        {isTransitioning && (
          <div className="absolute inset-0 bg-white/90 dark:bg-slate-800/90 rounded-2xl flex items-center justify-center z-10 animate-in fade-in duration-500 delay-200">
            <div className="text-center">
              <div className="animate-spin rounded-full h-10 w-10 border-3 border-teal-500 border-t-transparent mx-auto mb-4"></div>
              <p className="text-slate-600 dark:text-slate-400 text-sm font-medium">Returning to home...</p>
              <p className="text-slate-500 dark:text-slate-500 text-xs mt-1">Just a moment</p>
            </div>
          </div>
        )}
        ...
        <div className="flex space-x-3">
          <button
            ...
            className="flex-1 py-3 bg-gradient-to-r from-green-500 to-green-600 rounded-lg font-semibold text-white hover:scale-105 active:scale-95 transform transition-all duration-200 flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
          >
          ...
          <button
            ...
            className="flex-1 py-3 bg-slate-200 dark:bg-slate-700 rounded-lg text-slate-700 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-600 active:scale-95 transform transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          >
